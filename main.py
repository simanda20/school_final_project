...
while run:
    try:
        ...
        configuration = {  # json configuration pattern
            "web_service_address": "",  # address of web service
            "sleeping_time_hours": 0,  # sleeping time between cycles of data mining
        ...
        new_log_file = 'logs/' + str(date.today()) + '-' + str(datetime.now().hour) + "-" + str(
            datetime.now().minute) + "-" + str(datetime.now().second) + '.log'
        ...
        if exists("configuration.json"):  # check if configuration file exists
            ...
            if configuration["web_service_address"] != "":  # check if address works
                try:
                    ...
                    if req.status_code in range(200, 300):  # check server response
                        ...
                        miner.logging.error("Web service do not exist.")
                        ...
                        print("Web service is currently unavailable. We will try it again in next iteration")
                        configuration = True
                        service_access = False
            ...
            with open("configuration.json", "w+") as file:  # create configuration file
        ...
        if configurated:
            ...
            print(
                "Program was not configured properly. Please open configuration file and set up app before starting")
